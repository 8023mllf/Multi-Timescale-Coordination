...
def eval_policy(task_name,
    ...
    while succ_seed < test_num:
        ...
        TASK_ENV.setup_demo(now_ep_num=now_id, seed=now_seed, is_test=True, **args)
        # ✅ 只打印一次（避免每个 episode 都刷屏）
        if not hasattr(TASK_ENV, "_printed_obs_structure"):
            TASK_ENV._printed_obs_structure = True
            ...
            #     print("observation cameras:", list(obs["observation"].keys()))
            #     for cam, cam_dict in obs["observation"].items():
            #         if isinstance(cam_dict, dict):
            #             print(f"  - {cam}: {list(cam_dict.keys())}")
            #
            # if "joint_action" in obs and isinstance(obs["joint_action"], dict):
            #     print("joint_action keys:", list(obs["joint_action"].keys()))
            #     if "vector" in obs["joint_action"]:
            #         print("joint_action['vector']:", _brief(obs["joint_action"]["vector"]))
            # print("==== end ====\n")
