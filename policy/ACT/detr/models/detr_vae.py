...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder,
                 action_dim, qpos_dim, num_queries, camera_names):
        super().__init__()
...
def build(args):
    """
    对于原始 sim_* 任务：
        action_dim = qpos_dim = args.state_dim (默认 14)

    对于 BiGym 任务（task_name 里含有 'sim_bigym'）：
        action_dim = 15
        qpos_dim   = 63
    """
    default_dim = getattr(args, "state_dim", 14)

    task_name = getattr(args, "task_name", "")
    if "sim_bigym" in task_name:
        # 你现在的 MovePlate：/action 是 15 维，/observations/qpos 是 63 维
        action_dim = 15
        qpos_dim = 63
    else:
        # 保持原来的行为
        action_dim = default_dim
        qpos_dim = default_dim
